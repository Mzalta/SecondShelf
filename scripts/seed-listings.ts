...
const textbookBases: TextbookBase[] = [
...
function generateISBN(baseISBN: string): string {
  // Generate a variation by modifying the last few digits
...
function generateTags(hasSolutions: boolean, hasAccessCode: boolean, condition: string): string[] {
  const tags: string[] = []
...
function generateConditionText(condition: typeof conditions[0]): string {
...
async function seedListings() {
  ...
  try {
    // Idempotency check: skip if more than 50 listings exist
    ...
    if (count && count > 50) {
      console.log(`⏭️  Skipping seed: ${count} listings already exist (threshold: 50)`)
      console.log('   To re-seed, delete existing listings first.\n')
      return
    }

    console.log(`📊 Found ${count || 0} existing listings. Proceeding with seed...\n`)

    // Get existing users from profiles table for seller_id
    console.log('👥 Setting up seller users...')
    let sellerIds: string[] = []
    
    try {
      // Query profiles table to get existing user IDs
      ...
      if (!profileError && profileUsers && profileUsers.length > 0) {
        ...
        console.log(`   Found ${sellerIds.length} existing users in profiles table\n`)
      } else {
        console.warn('⚠️  No users found in profiles table')
        console.warn('   Attempting to query auth.users via RPC...\n')
        
        // Try to get users via a SQL function if available
        // Note: This may not work depending on RLS/function setup
        // If this fails, you'll need to create users first
      }
    } catch (error: any) {
      console.warn(`⚠️  Could not fetch existing users: ${error.message}`)
    }

    // If we don't have any users, we need to handle this
    if (sellerIds.length === 0) {
      console.error('❌ Error: No users found in the database!')
      console.error('   You need to create at least one user before seeding listings.')
      console.error('   The user_id field in books table requires valid user IDs.')
      console.error('\n   Options:')
      console.error('   1. Create users via your app\'s signup flow')
      console.error('   2. Create users manually in Supabase Dashboard → Authentication')
      console.error('   3. If user_id allows NULL, modify the script to use null values\n')
      throw new Error('No users found - cannot seed listings without valid user_ids')
    } else if (sellerIds.length < 5) {
      // If we have some users but not many, reuse them
      console.log(`   Will reuse ${sellerIds.length} users for multiple listings\n`)
    }

    // Generate listings
    console.log('📚 Generating textbook listings...')
    const listings: any[] = []

    for (const textbook of textbookBases) {
      // Generate 4-8 variations per textbook
      ...
      for (let i = 0; i < numVariations; i++) {
        ...
        const hasSolutions = Math.random() < 0.3 // 30% chance
        const hasAccessCode = Math.random() < 0.2 // 20% chance
        const price = random(condition.minPrice, condition.maxPrice)
        
        // Adjust price based on multipliers and extras
        ...
        // Generate 1-3 image URLs (using Lorem Picsum with seed for consistent images)
    ...
    for (let i = 0; i < listings.length; i += batchSize) {
      ...
      process.stdout.write(`   Inserted ${inserted}/${listings.length} listings...\r`)
    }

    console.log(`\n✅ Successfully inserted ${inserted} listings!\n`)
    console.log('🎉 Seeding complete!')
    console.log('\n💡 Note: If you see foreign key errors, you may need to create actual users')
    console.log('   in auth.users that match the generated user_ids.\n')
